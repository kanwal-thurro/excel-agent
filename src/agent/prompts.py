...
def create_match_selection_system_prompt(allow_no_match: bool = False):
    """
    ...
    Args:
        allow_no_match (bool): Whether to allow LLM to select "no_match" option
    ...
    # Conditionally add NO MATCH section
    if allow_no_match:
        no_match_section = """

    NO MATCH OPTION:
    - If NONE of the 5 matches are suitable for the target context, you can select "no_match"
    - Use this when:
      * Company names are completely different (e.g., target is "HDFC Bank" but all matches are different like "Reliance Industries")
      * Metric types are incompatible (e.g., target is standalone but all matches are about consolidated)
      * Time periods are too far apart (e.g., target is Q1 FY26 but all matches are from 2020)
      * Entity mismatch is too significant (e.g., target is "Retail Banking" but all matches are different like "Retail accounts")
      * Metric mismatch is too significant (e.g., target is "Net Interest Margin" but all matches are different like "Net Interest Income")
    - Be selective - only use "no_match" when there's a fundamental incompatibility
    - Small variations or minor mismatches should still result in selecting the best available option"""
    ...
    Return JSON with:
    {
        ...
        "reasoning": "Explain why this match is best based on the 5 criteria, or why no match is suitable",
        "confidence": <0.1-1.0>
    }
    """
    else:
    ...
    return base_prompt + no_match_section + json_format
...
def get_prompt_for_operation(operation_type: str, context: dict = None):
    ...
    if operation_type == "table_analysis":
        ...
        allow_no_match = context.get("allow_no_match", False) if context else False
        return (
            create_match_selection_system_prompt(allow_no_match=allow_no_match),
